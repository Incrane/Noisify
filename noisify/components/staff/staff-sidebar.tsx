...
import { LayoutDashboard, Building2, Calendar, BookOpen, DoorOpen, Medal, Trophy, Settings, LogOut, ChevronDown, Users, UserCog, BarChart3, MessageSquare, ArrowLeftRight, Gamepad2 } from "lucide-react";
...
export default function StaffSidebar({
  ...
  return (
    <aside className={cn("w-72 bg-white border-r border-slate-200/60 shrink-0 flex-col h-screen sticky top-0 z-50 shadow-sm hidden md:flex", className)}>
      {/* Header */}
      <div className="p-6 pb-4">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h1 className="text-xl font-bold text-slate-900 tracking-tight">Noisify</h1>
        </div>

        <Link
          href="/app/aktiviteter"
          onClick={handleLinkClick}
          className="group flex items-center justify-center w-full py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-medium transition-all shadow-sm hover:shadow-md active:scale-[0.98] text-sm"
        >
          Tillbaka till appen
        </Link>
      </div>
      ...
      <nav className="flex-1 px-3 space-y-1 overflow-y-auto py-2">
        <div className="px-3 py-2">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Översikt</p>
          <Link
            href="/staff"
            onClick={handleLinkClick}
        ...
        {isSuperAdmin && (
          <div className="px-3 py-2">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Super Admin</p>
            <Link
              href="/super_admin"
              onClick={handleLinkClick}
        ...
        <div className="px-3 py-2">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Administration</p>
          <div className="space-y-1">
            <div className="relative">
              <button
                onClick={() => setIsVerksamhetOpen(!isVerksamhetOpen)}
                className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-slate-600 hover:bg-slate-50 hover:text-slate-900 transition-all group text-left"
              >
                <span className="flex items-center gap-3">
                  <Building2 className="w-5 h-5 text-slate-400 group-hover:text-indigo-600 transition-colors" />
                  <span className="font-medium">Verksamhet</span>
                </span>
                <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform ${isVerksamhetOpen ? 'rotate-180' : ''}`} />
              </button>
            ...
            <Link href="/staff/kurser" onClick={handleLinkClick} className={linkClass('/staff/kurser')}>
              ...
              <span className="font-medium">Kurser</span>
            </Link>

            <Link href="/staff/rum" onClick={handleLinkClick} className={linkClass('/staff/rum')}>
              <DoorOpen className={iconClass('/staff/rum')} />
              <span className="font-medium">Rumsbokningar</span>
            </Link>
          </div>
        </div>

        <div className="px-3 py-2">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Verktyg</p>
          <div className="space-y-1">
            ...
            <Link href="#" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-all group cursor-not-allowed opacity-70">
      ...
      <div className="p-4 border-t border-slate-100 bg-slate-50/50">
        <div className="flex items-center gap-3 p-2 rounded-xl hover:bg-white hover:shadow-sm transition-all cursor-pointer group">
          <div className="w-10 h-10 bg-linear-to-br from-indigo-500 to-purple-600 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-sm ring-2 ring-white">
