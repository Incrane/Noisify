...
import { Check, ChevronRight, CreditCard, Info, Loader2, User, Users, Calendar } from "lucide-react";
import { toast } from "sonner";
import { applyForMembership } from "@/app/actions/membership";
...
export default function MembershipApplicationModal({
    ...
    const isEligible = (type: MembershipType) => {
        ...
        if (type.target_subgroups && type.target_subgroups.length > 0) {
            if (!profile.target_subgroup) return false;
            if (!type.target_subgroups.includes(profile.target_subgroup)) return false;
        }

        return true;
    };

    const visibleMembershipTypes = membershipTypes.filter(isEligible);
            ...
            <DialogContent className="sm:max-w-[600px] bg-white p-0 overflow-hidden gap-0">
                ...
                <div className="p-6 space-y-6">
                    <div className="space-y-3">
                        {visibleMembershipTypes.map((type) => {
