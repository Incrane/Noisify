-- Migration to fix address visibility security issue (v5 - fix all column names)
-- 1. Drop the view
DROP VIEW IF EXISTS public.v_explore_activities;

-- 2. Update the function with correct column names and logic
...
BEGIN
    ...
    SELECT 
        ...
        -- Replaced missing function with CASE statement
        CASE 
        ...
        END,
        ad.act_city_id,
        c.city, -- Fixed: c.name -> c.city
        o.id,
        o.org_namn, -- Fixed: o.name -> o.org_namn
        o.logo_url,
        ad.capacity,
        ad.reserve_capacity,
        ...
        (SELECT jsonb_agg(jsonb_build_object('id', cat.id, 'name', cat.cat_name, 'color', cat.color, 'bg_color', cat.bg_color)) -- Fixed: cat.name -> cat.cat_name
