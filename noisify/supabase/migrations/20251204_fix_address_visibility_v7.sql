-- Migration to fix address visibility security issue (v7 - fix all table and column names)
-- 1. Drop the view
DROP VIEW IF EXISTS public.v_explore_activities;

-- 2. Update the function with correct table/column names and logic
...
BEGIN
    ...
    SELECT 
        ...
        -- Replaced get_capacity_status with inline logic
        CASE
        ...
        (SELECT jsonb_agg(jsonb_build_object('id', cat.id, 'name', cat.cat_name, 'color', cat.color, 'bg_color', cat.bg_color)) 
         FROM activity_categories ac -- Fixed table name
         JOIN categories cat ON ac.category_id = cat.id
         WHERE ac.activity_id = ad.id),
        (SELECT array_agg(category_id) FROM activity_categories WHERE activity_id = ad.id), -- Fixed table name
        (SELECT jsonb_agg(jsonb_build_object('id', ts.id, 'name', ts.name))
         FROM activity_target_subgroups ats -- Fixed table name
         JOIN target_subgroups ts ON ats.sub_group_id = ts.id -- Fixed column name
         WHERE ats.activity_id = ad.id),
        ad.age_min,
        ad.age_max,
        -- Replaced get_age_range_text with inline logic
        CASE
