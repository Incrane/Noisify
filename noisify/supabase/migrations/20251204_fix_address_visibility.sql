-- Migration to fix address visibility security issue
-- 1. Drop the view that depends on the function
DROP VIEW IF EXISTS public.v_explore_activities;

-- 2. Update the function to redact address and return hide_address
...
BEGIN
    ...
    SELECT 
        ...
        get_activity_type_label(ad.type),
        ad.act_city_id,
        c.name,
        o.id,
        o.name,
        ...
        (SELECT jsonb_agg(jsonb_build_object('id', cat.id, 'name', cat.name, 'color', cat.color, 'bg_color', cat.bg_color))
         FROM activity_category ac
         JOIN categories cat ON ac.category_id = cat.id
         WHERE ac.activity_id = ad.id),
        (SELECT array_agg(category_id) FROM activity_category WHERE activity_id = ad.id),
        (SELECT jsonb_agg(jsonb_build_object('id', ts.id, 'name', ts.name))
         FROM activity_target_subgroup ats
         JOIN target_subgroups ts ON ats.target_subgroup_id = ts.id
         WHERE ats.activity_id = ad.id),
        ad.age_min,
        ad.age_max,
        get_age_range_text(ad.age_min, ad.age_max),
        user_reg.id,
        user_reg.status,
...
-- 3. Recreate the view with the new column definition
CREATE OR REPLACE VIEW public.v_explore_activities AS
 SELECT 
   ...
   FROM get_explore_activities(NULL::uuid) get_explore_activities(
    ...
    hide_address boolean -- Added this
   );
